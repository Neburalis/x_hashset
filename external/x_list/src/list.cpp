...
const list_containing_t POISON = {NULL, 0, 0, NULL};

int list_contairing_t_comparator(x_str::x_str_t first, x_str::x_str_t second) {
    return first.is_same(&second) ? 0 : 1;
}
...
list_t * constructor(size_t capacity) {
    ...
    for (size_t i = 1; i < capacity - 1; ++i) {
        ...
        list->elements[i].prev = SIZE_MAX;
    }

    list->elements[capacity - 1].next = 0; // После последнего ничего не идет
    list->elements[capacity - 1].prev = SIZE_MAX;
...
void erase(list_t *list, size_t element_idx) {
    ...
    elements[element_idx].prev = SIZE_MAX;
