...
#pragma clang diagnostic ignored "-Wformat"
    ...
    for (size_t i = 1; i < list->capacity; ++i) {
        if (elements[i].prev == SIZE_MAX) { // Это free элемент
            // printf("free node%zu\n", i);
            ...
            // printf("node%zu\n", i);
            fprintf(fp, "\tnode%zu [label=\"<index> index = %zu | <data> data = %s | { <prev> prev = %lld | <next> next = %zu}\", color = \"#ff9500ff\", fontcolor = \"black\", fillcolor = \"#ffd997\"];\n",
                        i, i, elements[i].data.str ? elements[i].data.str : "NULL", elements[i].prev, elements[i].next);
        }
    }
#pragma clang diagnostic pop
    ...
    for (size_t i = 0; i < list->capacity - list->size; ++i) {
        ...
        if (get_prev_element(list, el2) == SIZE_MAX || el2 == 0) {
...
void _dump_impl(list_t *list, FILE *logfile, const char *log_dirname,
    ...
    for (size_t i = 0; i < list->capacity; ++i) {
        if (i == 0)
            ...
            snprintf(CTPOKA, sizeof(CTPOKA), "%s", elements[i].data.str ? elements[i].data.str : "NULL");
