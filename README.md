...
`-O2`, `-O3` и `-Ofast` равны с точностью до погрешности (отличие +- 2%).
Дальше буду использовать `-O2` потому что такой же быстрый, при этом более стабильный, меньше риск раздувания кода из-за агресивного раскрытия циклов.

## Оптимизация 1: SoA-бакеты + AVX2 (`ветка opt/soa-buckets`)
...
~4.08B тактов - **в 1.68x быстрее** предыдущего этапа.
...
|---------|----------------:|
| `hash::crc32_hw` | **64.37%** |
| `bucket_soa::contains` | **25.31%** |
| `hashset_soa::contains` (overhead) | 6.11% |
| `__memcmp_avx2_movbe` | 0.77% |
...
~1.78B тактов суммарно - **в 2.29x быстрее** предыдущего этапа.
...
| Функция | % времени (self) |
|---------|----------------:|
| `bucket_soa::contains` | **54.08%** |
| `hash::crc32_hw` | **20.82%** |
| `hashset_soa::contains` (overhead) | 16.99% |
| `main` | 2.81% |
| `__memcmp_avx2_movbe` | 1.92% |
...
Но я всеравно сделал эту оптимизацию чтобы доказать теоретические выкладки

![](assets/image2.png)

~1.88B тактов что хуже на 5% от прошлого варианта
...
| Оптимизация 2 | SoA-бакеты + AVX2 | 4.08B | 3.21x | 1.68x |
| Оптимизация 1 | `crc32_hw` (SSE4.2) | 1.78B | **7.38x** | 2.29x |
| Оптимизация 3 | Аппаратное strcmp | 1.88B | 6.98x | 0.95x |

Итого две оптимизации дали **7.13x** ускорение: отключение верификатора (x2.0), аппаратный CRC32 (x1.4) и замена связных списков на SoA + AVX2 (x2.5).
А третий кандидат на оптимизацию только ухудшил ситуацию на примерно 5%.
