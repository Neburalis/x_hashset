...
uint64_t crc32_sw_raw(const char *str, size_t len) {
    uint32_t crc = 0xFFFFFFFFu;
...
uint64_t crc32_hw_aligned32(const char *slot) {
    const uint64_t *p = (const uint64_t *)__builtin_assume_aligned(slot, 32);
    ...
    return (crc ^ 0xFFFFFFFFull) & 0xFFFFFFFFull;
}

unsigned long crc32_hw_aligned(const x_str_t *s) {
    if (!s || !s->str) return 0;
    return (unsigned long)crc32_hw_aligned32(s->str);
}
